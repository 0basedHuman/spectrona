...
_MOCK_RESPONSE = {
    "id": "chatcmpl-spectrona-mock",
    "object": "chat.completion",
    "created": 0,
    "model": "gpt-4",
    "choices": [{
        "index": 0,
        "message": {"role": "assistant", "content": "[Mock response from Spectrona gateway]"},
        "finish_reason": "stop",
    }],
...
async def chat_completions(request: Request):
    ...
    try:
        upstream = await passthrough.forward_openai(
            "chat/completions",
            body_bytes,
            request.headers,
        )
        ...
        return JSONResponse({"error": f"OpenAI upstream request failed: {exc}"}, status_code=502)
